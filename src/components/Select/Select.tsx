...
const SelectDropDown = ({value, options, onChange, stateOpen, setOpened} : ISelectDropDownProps) => {
  ...
  return (
    <div ref={(ele) => myId.current = ele as any}
             className={`hw-select-choose-section ${stateOpen.visible === SELECT_STATE.SHOW ? 'select-show' : ''}`}
  ...
  return (
    <div className={`hw-select-root${className ? ` ${className}` : ''}`}>
      ...
      {opened.visible !== SELECT_STATE.CLOSE ?
        <SelectDropDown value={value} options={optionsSelect} onChange={onChange} stateOpen={opened}
