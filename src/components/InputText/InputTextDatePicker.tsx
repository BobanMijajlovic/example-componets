...
const InputTextDatePicker = (props : IInputTextDatePickerProps) => {
  ...
  const {onChange, format, locales} = props
  const handlerCalendarEvent = React.useCallback((data : ICalendarEventData, close ?: boolean) => {
    ...
    close && setStateCalendar('hide')
  }, [onChange, format, locales, setStateCalendar, checkFormat])
  ...
  return (
    <div className={'hw-input-text-date-picker'} hw-calendar-data='parent' onClick={handlerOnClick}
             ref={(ele) => uniqueId.current = ele as any}>
      <InputText
                {
                    ...props
                }
