    ...
    <>
      {
        arrayDays.map((arrRow, index) => {
          return (
            <div className={`hw-calendar-data-rows${index === 0 ? ' hw-calendar-header' : ''}`}
                             key={`row_${index}`}>
              {
                arrRow.map((cDay, ind) => {
                  ...
                  return (
                    <div
                                            className={`hw-calendar-panel-data${index === 0 ? ' hw-calendar-header' : ''}${cDay.accessible ? '' : ' disabled'}${active === calendarData ? ' active' : ''}`}
